...
void JAL() {
	...
	setRegister(rd, getPC() + 1);     // pc+4 bajty (d�ugo�� s�owa)
	setPC(getPC() + (imm / 4));         // trzeba chyba zmieni� spos�b adresowania PC na 4 na skok
}
...
void SB() {
	...
	storeDataWord(addr, getRegister(rs2) & 0xFF);        // 8-bit�w
